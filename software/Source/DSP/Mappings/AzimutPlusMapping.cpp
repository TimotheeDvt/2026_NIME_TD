#include "AzimutPlusMapping.h"
#include "../BoStaffSynth.h"
#include "../../UI/DebugLog.h"
#include <cmath>
#include <algorithm>

constexpr float AzimutPlusMapping::kRootSemitoneTable[2][2][2];
constexpr float AzimutPlusMapping::kChordVoicing[3];

AzimutPlusMapping::AzimutPlusMapping()
    : laban_weight_monitor_(addMonitorParam("Drive", "Laban Weight", 0.0f, 1.0f)),
      ...
      filter_cutoff_monitor_(addMonitorParam("LPF Cutoff", "Speed", 20.0f, 20000.0f)),
...
void AzimutPlusMapping::prepare(double sample_rate_hz) {
    debug.print.green("AzimutPlusMapping prepared at sample rate:", sample_rate_hz);
...
void AzimutPlusMapping::calculateDeltaTime() {
...
void AzimutPlusMapping::updateTipPositionHistory(const StaffSoundParams& input_parameters, float& current_tip_x, float& current_tip_y, float& current_tip_z) {
...
void AzimutPlusMapping::calculateRotationAxisAtMidpoint(float& axis_x, float& axis_y, float& axis_z) {
...
void AzimutPlusMapping::updateGravityVector(const StaffSoundParams& input_parameters) {
...
void AzimutPlusMapping::calculateDynamicAcceleration(const StaffSoundParams& input_parameters, float& dynamic_accel_x, float& dynamic_accel_y, float& dynamic_accel_z, float& dynamic_accel_magnitude) {
...
float AzimutPlusMapping::integrateVelocityForLabanWeight(float dynamic_accel_x, float dynamic_accel_y, float dynamic_accel_z) {
...
void AzimutPlusMapping::detectAxialThrustPeaks(const StaffSoundParams& input_parameters, float dynamic_accel_x, float dynamic_accel_y, float dynamic_accel_z, float tip_x, float tip_y, float tip_z, float gyroscope_magnitude) {
...
float AzimutPlusMapping::updateLabanWeight(float velocity_magnitude) {
...
float AzimutPlusMapping::updateLabanTime(float gyroscope_magnitude) {
...
float AzimutPlusMapping::updateLabanSpace(const StaffSoundParams& input_parameters, float gyroscope_magnitude) {
...
void AzimutPlusMapping::updateLabanFlow(float dynamic_acceleration_magnitude, float& flow_bound, float& flow_free) {
...
bool AzimutPlusMapping::updateSpinClassification(float axis_x, float axis_y, float axis_z) {
...
bool AzimutPlusMapping::updateFacingClassification(float tip_x, float tip_y) {
...
void AzimutPlusMapping::applyPitchAndChordToOutput(MappingOutput& mapping_output, const StaffSoundParams& input_parameters) {
...
void AzimutPlusMapping::applyVoicesToOutput(MappingOutput& mapping_output, float melody_gain) {
...
void AzimutPlusMapping::applyMasterGainToOutput(MappingOutput& mapping_output, float laban_weight, float motion_gate) {
...
void AzimutPlusMapping::applyTimbreToOutput(MappingOutput& mapping_output, float laban_space_focus, float laban_weight) {
...
void AzimutPlusMapping::applyNoiseToOutput(MappingOutput& mapping_output, float suddenness_normalized) {
...
void AzimutPlusMapping::applyModulationToOutput(MappingOutput& mapping_output, float laban_weight, float flow_bound, float flow_free) {
...
void AzimutPlusMapping::applyStereoPanToOutput(MappingOutput& mapping_output, float flow_free, float axis_x, float axis_y) {
...
void AzimutPlusMapping::process(const StaffSoundParams& input_parameters, MappingOutput& mapping_output) {
    calculateDeltaTime();
    ...
    // Cutoff tracks the staff's rotation speed directly, rather than the
    // number of continuous spins in the same direction (see AzimutMapping).
    ...
    float target_lpf_cutoff = juce::jmap(speed_normalized, 0.0f, 1.0f, 400.0f, 20000.0f);
