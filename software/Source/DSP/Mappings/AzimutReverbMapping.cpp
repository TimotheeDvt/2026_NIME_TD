...
void AzimutReverbMapping::applyPitchAndChordToOutput(MappingOutput& mapping_output, const StaffSoundParams& input_parameters) {
    ...
    const int facing_index = motion_.isFacingNorth() ? 0 : 1;
...
void AzimutReverbMapping::process(const StaffSoundParams& input_parameters, MappingOutput& mapping_output) {
    ...
    thrust_peak_monitor_.value.store(motion_.axialThrustPeakEnvelope(), std::memory_order_relaxed);
    ...
    if (is_moving) {
        spin_changed = motion_.updateSpinClassificationByAbsoluteComponent(rotation_axis_x, rotation_axis_y, rotation_axis_z);
        facing_changed = motion_.updateFacingClassification(tip_x, tip_y);
    ...
    if (spin_changed || facing_changed) {
        ...
        debug.print.cyan(
            ...
            motion_.isFacingNorth() ? "NORTH" : "EAST",
