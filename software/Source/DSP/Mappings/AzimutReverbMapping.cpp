#include "AzimutReverbMapping.h"
#include "../BoStaffSynth.h"
#include "../../UI/DebugLog.h"
#include <cmath>
#include <algorithm>

constexpr float AzimutReverbMapping::kRootSemitoneTable[2][2][2];
constexpr float AzimutReverbMapping::kChordVoicing[3];

AzimutReverbMapping::AzimutReverbMapping()
    : laban_weight_monitor_(addMonitorParam("Drive", "Laban Weight", 0.0f, 1.0f)),
      ...
      reverb_wet_monitor_(addMonitorParam("Reverb", "Laban Flow (Free)", 0.0f, 1.0f))
{
}

void AzimutReverbMapping::prepare(double sample_rate_hz) {
    debug.print.green("AzimutReverbMapping prepared at sample rate:", sample_rate_hz);
...
void AzimutReverbMapping::calculateDeltaTime() {
...
void AzimutReverbMapping::updateTipPositionHistory(const StaffSoundParams& input_parameters, float& current_tip_x, float& current_tip_y, float& current_tip_z) {
...
void AzimutReverbMapping::calculateRotationAxisAtMidpoint(float& axis_x, float& axis_y, float& axis_z) {
...
void AzimutReverbMapping::updateGravityVector(const StaffSoundParams& input_parameters) {
...
void AzimutReverbMapping::calculateDynamicAcceleration(const StaffSoundParams& input_parameters, float& dynamic_accel_x, float& dynamic_accel_y, float& dynamic_accel_z, float& dynamic_accel_magnitude) {
...
float AzimutReverbMapping::integrateVelocityForLabanWeight(float dynamic_accel_x, float dynamic_accel_y, float dynamic_accel_z) {
...
void AzimutReverbMapping::detectAxialThrustPeaks(const StaffSoundParams& input_parameters, float dynamic_accel_x, float dynamic_accel_y, float dynamic_accel_z, float tip_x, float tip_y, float tip_z, float gyroscope_magnitude) {
...
float AzimutReverbMapping::updateLabanWeight(float velocity_magnitude) {
...
float AzimutReverbMapping::updateLabanTime(float gyroscope_magnitude) {
...
float AzimutReverbMapping::updateLabanSpace(const StaffSoundParams& input_parameters, float gyroscope_magnitude) {
...
void AzimutReverbMapping::updateLabanFlow(float dynamic_acceleration_magnitude, float& flow_bound, float& flow_free) {
...
bool AzimutReverbMapping::updateSpinClassification(float axis_x, float axis_y, float axis_z) {
...
bool AzimutReverbMapping::updateFacingClassification(float tip_x, float tip_y) {
...
void AzimutReverbMapping::applyPitchAndChordToOutput(MappingOutput& mapping_output, const StaffSoundParams& input_parameters) {
...
void AzimutReverbMapping::applyVoicesToOutput(MappingOutput& mapping_output, float melody_gain) {
...
void AzimutReverbMapping::applyMasterGainToOutput(MappingOutput& mapping_output, float laban_weight, float motion_gate) {
...
void AzimutReverbMapping::applyTimbreToOutput(MappingOutput& mapping_output, float laban_space_focus, float laban_weight) {
...
void AzimutReverbMapping::applyNoiseToOutput(MappingOutput& mapping_output, float suddenness_normalized) {
...
// Bozendo/Azimut/Azimut+ all compute Laban Flow (flow_bound = tense/restrained
// motion, flow_free = loose/unrestrained motion) and either discard it or use
// it only for vibrato/pan. Here flow_free directly drives a reverb send: free
// motion opens a longer, brighter tail; bound motion collapses it back toward
// a short, damped, mostly-dry space. Weight scales how far the tail can open
// so a loose but tiny gesture doesn't flood the sound.
void AzimutReverbMapping::applyModulationToOutput(MappingOutput& mapping_output, float laban_weight, float flow_bound, float flow_free) {
    juce::ignoreUnused(flow_bound);
    ...
    mapping_output.reverbWetLevel = juce::jlimit(0.0f, 1.0f, flow_free * (0.25f + laban_weight * 0.75f));
    mapping_output.reverbRoomSize = juce::jmap(flow_free, 0.0f, 1.0f, 0.25f, 0.95f);
    mapping_output.reverbDamping  = juce::jmap(flow_free, 0.0f, 1.0f, 0.80f, 0.15f);

    reverb_wet_monitor_.value.store(mapping_output.reverbWetLevel, std::memory_order_relaxed);
}

void AzimutReverbMapping::applyStereoPanToOutput(MappingOutput& mapping_output, float flow_free, float axis_x, float axis_y) {
...
void AzimutReverbMapping::process(const StaffSoundParams& input_parameters, MappingOutput& mapping_output) {
    calculateDeltaTime();
