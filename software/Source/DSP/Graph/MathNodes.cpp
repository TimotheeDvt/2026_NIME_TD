...
void addStateless(NodeTypeRegistry& registry, const char* id, const char* name, int numInputs,
...
void addStateful(NodeTypeRegistry& registry, const char* id, const char* name, int numInputs,
                  std::vector<float> defaultParams, NodeTypeInfo::MathEvalFn eval) {
    NodeTypeInfo info;
    info.id = id;
    info.displayName = name;
...
void registerMathNodes(NodeTypeRegistry& registry) {
    ...
    addStateless(registry, "math.lut3", "Lookup Table (3 selectors)", 3, std::vector<float>(8, 0.0f),
        [](const float* in, int, const std::vector<float>& p, NodeState*, float* out) {
            if (p.empty()) { out[0] = 0.0f; return; }
    ...
    // pitch morph speed).
    addStateful(registry, "math.onePoleSmoother", "One-Pole Smoother", 2, { 0.1f },
    ...
    addStateful(registry, "math.lfoSine", "LFO (Sine)", 0, { 5.0f },
    ...
    addStateful(registry, "math.leakyIntegrator", "Leaky Integrator", 1, { 0.99f },
    ...
    addStateful(registry, "math.retriggerEnvelope", "Retrigger Envelope", 1, { 0.9f },
        [](const float* in, int, const std::vector<float>& p, NodeState* state, float* out) {
            auto* s = static_cast<MathNodeState*>(state);
    ...
    addStateful(registry, "math.hysteresisStep", "Hysteresis Step", 1, { 0.5f },
        [](const float* in, int, const std::vector<float>& p, NodeState* state, float* out) {
            auto* s = static_cast<MathNodeState*>(state);
    ...
    addStateful(registry, "math.derivative", "Derivative", 1, {},
        [](const float* in, int, const std::vector<float>&, NodeState* state, float* out) {
            auto* s = static_cast<MathNodeState*>(state);
    ...
    addStateful(registry, "math.latchedSmoother", "Latched Smoother", 2, { 0.1f },
        [](const float* in, int, const std::vector<float>& p, NodeState* state, float* out) {
            auto* s = static_cast<MathNodeState*>(state);
