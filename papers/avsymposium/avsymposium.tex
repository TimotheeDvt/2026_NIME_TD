...
continuously through the first author's own development and rehearsal; what it is not
yet grounded in is anyone else's experience of the instrument, which is precisely the
gap the evaluation in \S\ref{sec:evaluation} is intended to close. Until that session
runs, the claims above should be read as implementation-level evidence, not validated
usability.
...
One extension is next on the list regardless of the planned evaluation's outcome:
...
unit through to three distinct synthesis voices (\S\ref{sec:system}); what it has not
yet established is how the instrument plays in hands other than its designer's, which is
the question the evaluation in \S\ref{sec:evaluation} and the ESP-NOW rework in
\S\ref{sec:discussion} are both aimed at moving closer to answering.
